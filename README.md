...
The _'ChargingState'_ characteristic has the permission to `notify` the HomeKit controller of state 
changes. `homebridge-tesla-gateway` supports two concepts to send status changes to HomeKit.

### The 'pull' way:

The 'pull' way is probably the easiest to set up and supported in every scenario. `homebridge-tesla-gateway` 
requests the status of the sensor in an specified interval (pulling) and sends the value to HomeKit.  
Look for `pullInterval` in the list of configuration options if you want to configure it.

### The 'push' way:

When using the 'push' concept, the http device itself sends the updated value to `homebridge-tesla-gateway` 
whenever values change. This is more efficient as the new value is updated instantly and 
`homebridge-tesla-gateway` does not need to make needless requests when the value didn't actually change.  
However because the http device needs to actively notify the `homebridge-tesla-gateway` there is more 
...
* `patternGroupToExtract` \<number\> **optional** \(Default: **1**\): Defines the regex group of which the status 
    is extracted.
* `statusCache` \<number\> **optional** \(Default: **0**\): Defines the amount of time in milliseconds a queried value 
   of the _CurrentStatus_ characteristic is cached before a new request is made to the http device.  
...
`homebridge-tesla-gateway` can be used together with 
...
Down here is an example on how to configure `homebridge-tesla-gateway` to work with your implementation of the 
...
* `characteristic` "CurrentStatus": expects an float `value` by default.
